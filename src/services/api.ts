...
export const api = {
  ...
  async getAvailableFeatureColumns(floorId: string): Promise<string[]> {
    ...
    try {
      ...
      // Filter columns that have the value 'present' or 'unverified'
      const availableFeatures = Object.keys(data).filter(key => data[key] === 'present' || data[key] === 'unverified');
      
      console.log('[DEBUG] API - Available feature columns:', availableFeatures);
  ...
  async getAccessibilityFeatures(buildingId: string, floorId: string): Promise<AccessibilityFeature[]> {
    ...
    try {
      ...
      // Fetch active reports for this floor to sync unverified/reported confidence ratings and reporter counts
      ...
      if (reportsData && Array.isArray(reportsData)) {
        reportsData.forEach(r => {
          if (r.feature_type) {
            reportsMap.set(r.feature_type, r);
          }
        });
      }

      // Iterate over columns to find 'present', 'temporary', 'unverified', or other statuses
      Object.keys(data).forEach((key) => {
        const val = data[key];
        if ((val === 'present' || val === 'temporary' || val === 'unverified' || val === 'broken') && geometry && geometry[key]) {
          const geom = geometry[key];
          const isUnverified = val === 'unverified';
          const matchedReport = reportsMap.get(key);

          // Get confidence score, level, and reporter count directly from matching reports table if available
          let confScore = isUnverified ? 40 : 100;
          let confLevel: ConfidenceLevel = isUnverified ? 'LOW' : 'HIGH';
          let repCount = isUnverified ? 1 : 0;

          if (matchedReport) {
            if (matchedReport.confidence_score !== null && matchedReport.confidence_score !== undefined) {
              confScore = Number(matchedReport.confidence_score);
            }
            if (matchedReport.confidence_level) {
              confLevel = matchedReport.confidence_level as ConfidenceLevel;
            }
            if (matchedReport.reporter_count !== null && matchedReport.reporter_count !== undefined) {
              repCount = Number(matchedReport.reporter_count);
            }
          }
          ...
          features.push({
            ...
            status: isUnverified ? 'unverified' : val === 'temporary' ? 'temporary' : val === 'broken' ? 'broken' : 'working',
            x: geom.x, // Absolute 1600x800 coordinate
            y: geom.y, // Absolute 1600x800 coordinate
            width: geom.width,
            height: geom.height,
            description: `Accessibility feature: ${key} (${val})`,
            confidenceScore: confScore,
            confidenceLevel: confLevel,
            reporterCount: repCount,
            featureKey: key,
            verificationStatus: isUnverified ? 'unverified' : 'verified',
            lastUpdated: matchedReport?.updated_at || matchedReport?.created_at || new Date().toISOString()
          });
        }
      });
